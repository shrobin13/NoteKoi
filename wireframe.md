# UI/UX Plan — Resource Sharing Platform (v2)
...
- **Figma / Apple** — contextual toolbars that appear only when relevant, and disappear when not. We borrow this for resource-detail actions (report, recommend-for-promotion, edit) which are role- and state-dependent and must never show an action the current viewer cannot take.
...
- **Discover** hierarchy: (1) search bar, (2) "continue where you left off" recents strip, (3) Department → Session → Course browse entry, (4) a light "recently approved" or "platform highlights" feed — never buried below a hero banner.
