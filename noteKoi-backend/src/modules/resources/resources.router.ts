...
router.get(
  "/public",
  validateWithDetails(PublicResourceQuerySchema, "query"),
  ctrl.getPublicResources,
);
...
router.get(
  ...
  requireScope("classroomUnit"),
  validateWithDetails(PrivateResourceQuerySchema, "query"),
  ctrl.getUnitResources,
);
...
router.post(
  ...
  requireRole(Role.CR),
  requireScope("classroomUnit"),
  validateWithDetails(CreateResourceSchema),
  ctrl.createResource,
);
...
router.patch(
  ...
  authenticate({ requireVerified: true }),
  requireRole(Role.CR),
  validateWithDetails(UpdateResourceSchema),
  ctrl.updateResource,
);
