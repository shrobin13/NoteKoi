 ...
 */
export async function assignCR(dto: AssignCRDto, actorId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (existingSeat) {
      throw ApiError.conflict(
        `The ${dto.seat} seat for this ClassroomUnit is already occupied`,
        "CR_SEAT_TAKEN",
      );
    }
    ...
    if (userExistingAssignment) {
      throw ApiError.conflict(
        "User already holds a CR seat in this ClassroomUnit",
        "USER_ALREADY_CR",
      );
    }
 ...
 */
export async function demoteCR(dto: DemoteCRDto, actorId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (!assignment) throw ApiError.notFound("No active CR assignment found");
...
export async function listCRsForUnit(classroomUnitId: string) {
