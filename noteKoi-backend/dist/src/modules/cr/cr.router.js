...
router.get("/:classroomUnitId", authenticate(), ctrl.listCRsForUnit);
// POST /api/cr/assign — Sub Admin (own college) or Owner Admin (R-017, INV-003)
router.post("/assign", authenticate(), requireRole(Role.SUB_ADMIN, Role.OWNER_ADMIN), requireScope("college"), validateWithDetails(AssignCRSchema), ctrl.assignCR);
// DELETE /api/cr/demote — Sub Admin (own college) or Owner Admin (R-017)
router.delete("/demote", authenticate(), requireRole(Role.SUB_ADMIN, Role.OWNER_ADMIN), requireScope("college"), validateWithDetails(DemoteCRSchema), ctrl.demoteCR);
