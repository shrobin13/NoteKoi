 ...
 */
export async function assignSubAdmin(dto, actorId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (existing) {
            throw ApiError.conflict("This college already has an active Sub Admin. Demote them first.", "SUB_ADMIN_SEAT_TAKEN");
        }
        // Promote user role
 ...
 */
export async function demoteSubAdmin(userId, actorId) {
    return prisma.$transaction(async (tx) => {
        const assignment = await tx.adminAssignment.findFirst({
            where: { userId, role: AdminRole.SUB_ADMIN, isActive: true },
        });
        if (!assignment)
            throw ApiError.notFound("No active Sub Admin assignment found for this user");
        // Revoke assignment
 ...
 */
export async function transferOwnership(dto, actorId) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!currentAssignment)
            throw ApiError.internal("Could not find current Owner Admin assignment");
        await tx.adminAssignment.update({
...
export async function getPlatformStats() {
    const [userCount, verifiedUsers, pendingUsers, collegeCount, departmentCount, classroomUnitCount, resourceCount, personalShareCount,] = await prisma.$transaction([
        prisma.user.count(),
