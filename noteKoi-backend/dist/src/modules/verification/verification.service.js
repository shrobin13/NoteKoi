 ...
 */
export async function requestVerification(userId, dto) {
 ...
 */
export async function approveVerification(requestId, approverId) {
...
async function validateApproverScope(approver, unit, unitCollegeId) {
    // Owner Admin: platform-wide fallback — no scope restriction
    if (approver.role === Role.OWNER_ADMIN)
        return;
 ...
 */
export async function getPendingRequests(approverId, approverRole, approverCollegeId, approverClassroomUnitId, pagination) {
    const { skip, take } = paginate(pagination.page, pagination.limit);
    let whereClause = { status: VerificationStatus.PENDING };
