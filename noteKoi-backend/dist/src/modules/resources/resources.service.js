 ...
 */
export async function getPublicResources(query) {
 ...
 */
export async function getUnitResources(classroomUnitId, query) {
    const { skip, take } = paginate(query.page, query.limit);
    const where = {
        classroomUnitId,
    };
 ...
 */
export async function createResource(dto, uploaderId) {
 ...
 */
export async function updateResource(id, dto, classroomUnitId) {
 ...
 */
export async function deleteResource(id, classroomUnitId) {
    const resource = await prisma.resource.findUnique({ where: { id } });
    if (!resource)
        throw ApiError.notFound("Resource not found");
    if (resource.classroomUnitId !== classroomUnitId) {
