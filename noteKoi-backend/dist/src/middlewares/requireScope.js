 ...
 */
export function requireScope(scopeType) {
    return async (req, _res, next) => {
        if (!req.user)
            return next(ApiError.unauthorized());
        // Owner Admin has no scope restriction (ai-context.md §3)
        if (req.user.role === Role.OWNER_ADMIN)
            return next();
        try {
...
async function requireClassroomUnitScope(req, next) {
    const targetId = getParam(req, "classroomUnitId") ||
        req.body["classroomUnitId"];
    if (!targetId || typeof targetId !== "string") {
        return next(ApiError.badRequest("classroomUnitId is required for scope check", "SCOPE_PARAM_MISSING"));
    }
    if (req.user.role === Role.CR) {
        ...
        if (!assignment) {
            return next(ApiError.forbidden("You do not have CR rights over this classroom unit"));
        }
        return next();
    }
    ...
    if (req.user.role === Role.SUB_ADMIN) {
        ...
        if (!unit)
            return next(ApiError.notFound("ClassroomUnit not found"));
        if (unit.department.collegeId !== req.user.collegeId) {
            return next(ApiError.forbidden("This classroom unit is outside your college scope"));
        }
        return next();
    }
    return next(ApiError.forbidden());
}
async function requireCollegeScope(req, next) {
    const targetCollegeId = getParam(req, "collegeId") ||
        req.body["collegeId"];
    if (!targetCollegeId || typeof targetCollegeId !== "string") {
        return next(ApiError.badRequest("collegeId is required for scope check", "SCOPE_PARAM_MISSING"));
    }
    if (req.user.role === Role.SUB_ADMIN) {
        ...
        if (!assignment) {
            return next(ApiError.forbidden("This college is outside your admin scope"));
        }
        return next();
    }
    return next(ApiError.forbidden());
